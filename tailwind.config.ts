import { nextui } from "@nextui-org/theme";
import type { Config } from "tailwindcss";

const config: Config = {
  darkMode: ["class", '[data-mode="dark"]'],
  content: [
    "./src/**/*.{js,ts,jsx,tsx,mdx}",
    "./node_modules/@nextui-org/theme/dist/components/(date-picker|button|ripple|spinner|calendar|date-input|popover).js",
  ],
  theme: {
    extend: {
      colors: {
        primary: {
          10: "var(--k-color-primary-10)",
          25: "var(--k-color-primary-25)",
          50: "var(--k-color-primary-50)",
          100: "var(--k-color-primary-100)",
          200: "var(--k-color-primary-200)",
          300: "var(--k-color-primary-300)",
          400: "var(--k-color-primary-400)",
          500: "var(--k-color-primary-500)",
          600: "var(--k-color-primary-600)",
          700: "var(--k-color-primary-700)",
          800: "var(--k-color-primary-800)",
          900: "var(--k-color-primary-900)",
        },
        primary2: "var(--k-color-primary2)",
        secondary: "var(--k-color-secondary)",
        accent: "var(--k-color-accent)",
        background: "var(--k-color-background)",
        surface: "var(--k-color-surface)",
        text: {
          10: "var(--k-color-text-10)",
          50: "var(--k-color-text-50)",
          100: "var(--k-color-text-100)",
          200: "var(--k-color-text-200)",
          300: "var(--k-color-text-300)",
          400: "var(--k-color-text-400)",
          500: "var(--k-color-text-500)",
          600: "var(--k-color-text-600)",
          700: "var(--k-color-text-700)",
          800: "var(--k-color-text-800)",
          900: "var(--k-color-text-900)",
        },
        kakrola: {
          10: "#f7fbfd",
          25: "#f0f7fa",
          50: "#e1eff5",
          100: "#c3dfeb",
          200: "#a5cfe1",
          300: "#87bfd7",
          400: "#69afcd",
          500: "#005c83",
          600: "#004d6e",
          700: "#003e59",
          800: "#002f44",
          900: "#00202f",
        },
        // Moonstone color
        moonstone: {
          10: "#f2f4f8",
          25: "#e6e9ef",
          50: "#ccd3df",
          100: "#99a7bf",
          200: "#66799f",
          300: "#445a7f",
          400: "#3c506e",
          500: "#39485e",
          600: "#313e54",
          700: "#2a3547",
          800: "#222b3a",
          900: "#1b2230",
        },
        // Warning color
        tangerine: {
          10: "#fcf8f2",
          25: "#faf2e6",
          50: "#f5e5cd",
          100: "#ebcb9b",
          200: "#e1b169",
          300: "#d79737",
          400: "#cd7d05",
          500: "#d17c00",
          600: "#af6800",
          700: "#8d5400",
          800: "#6b4000",
          900: "#492c00",
        },
        // Success color
        kale: {
          10: "#f3f7f3",
          25: "#e8efe8",
          50: "#d1dfd1",
          100: "#a3bfa3",
          200: "#759f75",
          300: "#477f47",
          400: "#195f19",
          500: "#297b2d",
          600: "#226726",
          700: "#1b531f",
          800: "#143f18",
          900: "#0d2b11",
        },
        lavender: {
          10: "#f8f6ff",
          25: "#f1edff",
          50: "#e3dbff",
          100: "#c7b7ff",
          200: "#ab93ff",
          300: "#8f6fff",
          400: "#734bff",
          500: "#6c40ff",
          600: "#5b36d6",
          700: "#4a2cad",
          800: "#392284",
          900: "#28185b",
        },
        // Danger color
        raspberry: {
          10: "#fef2f5",
          25: "#fce6eb",
          50: "#f9cdd7",
          100: "#f39bb0",
          200: "#ed6988",
          300: "#e73760",
          400: "#e10538",
          500: "#d1003b",
          600: "#af0032",
          700: "#8d0029",
          800: "#6b0020",
          900: "#490017",
        },
        gray: {
          10: "#f9f9f9",
          25: "#f2f2f2",
          50: "#ebebeb",
          100: "#d9d9d9",
          200: "#bfbfbf",
          300: "#a6a6a6",
          400: "#8c8c8c",
          500: "#737373",
          600: "#5b5b5b",
          700: "#424242",
          800: "#292929",
          900: "#111111",
        },

        border: "hsl(var(--border))",
        input: "hsl(var(--input))",
        ring: "hsl(var(--ring))",
        foreground: "hsl(var(--foreground))",
        destructive: {
          DEFAULT: "hsl(var(--destructive))",
          foreground: "hsl(var(--destructive-foreground))",
        },
        popover: {
          DEFAULT: "hsl(var(--popover))",
          foreground: "hsl(var(--popover-foreground))",
        },
        card: {
          DEFAULT: "hsl(var(--card))",
          foreground: "hsl(var(--card-foreground))",
        },
      },
      keyframes: {
        "accordion-down": {
          from: { height: "0" },
          to: { height: "var(--radix-accordion-content-height)" },
        },
        "accordion-up": {
          from: { height: "var(--radix-accordion-content-height)" },
          to: { height: "0" },
        },
      },
      animation: {
        "accordion-down": "accordion-down 0.2s ease-out",
        "accordion-up": "accordion-up 0.2s ease-out",
      },
    },
  },
  safelist: [
    "bg-primary-10",
    "bg-primary-25",
    "bg-primary-50",
    "bg-primary-100",
    "bg-primary-200",
    "bg-primary-300",
    "bg-primary-400",
    "bg-primary-500",
    "bg-primary-600",
    "bg-primary-800",
    "bg-primary-900",
    "bg-primary-900",
    "text-primary-500",

    "bg-kakrola-10",
    "bg-kakrola-25",
    "bg-kakrola-50",
    "bg-kakrola-100",
    "bg-kakrola-200",
    "bg-kakrola-400",
    "bg-kakrola-500",
    "bg-kakrola-500/20",
    "bg-kakrola-600",
    "bg-kakrola-800",
    "bg-kakrola-900",
    "text-kakrola-500",

    "bg-tangerine-10",
    "bg-tangerine-25",
    "bg-tangerine-50",
    "bg-tangerine-100",
    "bg-tangerine-200",
    "bg-tangerine-400",
    "bg-tangerine-500",
    "bg-tangerine-500/20",
    "bg-tangerine-600",
    "bg-tangerine-800",
    "bg-tangerine-900",
    "text-tangerine-500",

    "bg-kale-10",
    "bg-kale-25",
    "bg-kale-50",
    "bg-kale-100",
    "bg-kale-200",
    "bg-kale-400",
    "bg-kale-500",
    "bg-kale-500/20",
    "bg-kale-600",
    "bg-kale-800",
    "bg-kale-900",
    "text-kale-500",

    "bg-lavender-10",
    "bg-lavender-25",
    "bg-lavender-50",
    "bg-lavender-100",
    "bg-lavender-200",
    "bg-lavender-400",
    "bg-lavender-500",
    "bg-lavender-500/20",
    "bg-lavender-600",
    "bg-lavender-800",
    "bg-lavender-900",
    "text-lavender-500",

    "bg-raspberry-10",
    "bg-raspberry-25",
    "bg-raspberry-50",
    "bg-raspberry-100",
    "bg-raspberry-200",
    "bg-raspberry-400",
    "bg-raspberry-500",
    "bg-raspberry-500/20",
    "bg-raspberry-600",
    "bg-raspberry-800",
    "bg-raspberry-900",
    "text-raspberry-500",

    "bg-gray-10",
    "bg-gray-25",
    "bg-gray-50",
    "bg-gray-100",
    "bg-gray-200",
    "bg-gray-400",
    "bg-gray-500",
    "bg-gray-600",
    "bg-gray-800",
    "bg-gray-900",
    "bg-gray-900/20",
    "text-gray-500",

    "text-indigo-500",
    "text-pink-500",
  ],
  plugins: [
    nextui(),
    require("tailwindcss-animate"),
    require("@tailwindcss/typography"),
  ],
};

export default config;
